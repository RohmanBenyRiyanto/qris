...
/// retrieved data into a strongly-typed `MccData` object. The data is statically
...
/// Usage:
/// ```dart
/// final mccHelper = MCCHelper();
/// Map<String, dynamic> mccInfo = mccHelper.getMCCByCode("0742");
/// MccData mccDataObject = mccHelper.mccCodeToObject("0742");
/// ```
///
...
class MCCHelper {
  ...
  MCCHelper._privateConstructor();

  // The singleton instance of the MCCHelper class.
  static final MCCHelper _instance = MCCHelper._privateConstructor();

  /// Factory constructor to return the single instance of MCCHelper.
  ///
  /// This ensures that there is only one instance of `MCCHelper` throughout the app.
  factory MCCHelper() {
    return _instance;
  }
  ...
  static Map<String, dynamic> getMCCByCode(String mccCode) {
    return _mccData.firstWhere(
      (item) => item['mcc'] == mccCode,
      orElse: () => {},
    );
  }

  /// Converts the MCC data for the provided MCC code into an `MccData` object.
  ///
  /// This method converts the MCC data map returned by `getMCCByCode` into a strongly-typed
  /// `MccData` object using the `MccData.fromJson` constructor.
  ///
  /// Example:
  /// ```dart
  /// MccData mccData = MCCHelper().mccCodeToObject("0742");
  /// ```
  ///
  /// Arguments:
  /// - `mccCode`: The MCC code as a `String` (e.g., `"0742"`).
  ///
  /// Returns:
  /// - `MccData?`: The `MccData` object corresponding to the given `mccCode`, or `null` if not found.
  static MccData? mccCodeToObject(String mccCode) {
    return MccData.fromJson(getMCCByCode(mccCode));
  }
}

/// A list of MCC data stored in-memory.
///
/// This is a static list that represents the MCC data. The data here is based on the MCC data
/// extracted from the [MCC Code GitHub Repository](https://github.com/greggles/mcc-codes).
///
/// Example data:
/// ```json
/// {
...
/// }
/// ```
final List<Map<String, dynamic>> _mccData = [
  {
    "mcc": "0742",
    "edited_description": "Veterinary Services",
    "combined_description": "Veterinary Services",
